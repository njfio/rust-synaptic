...
**RESOLVED (v3, Task B):** `AgentMemory` now retains the concrete `Arc<TfIdfProvider>` shared by the dense retriever and reranker (`retrieval_scoring_provider`) and `store_with_report` feeds every stored memory's content into it via the document `embed` path (best-effort, O(tokens) per store). The provider's scoring cache self-invalidates on each ingest, so query-time vectors always reflect current corpus IDF. Proven by `tests/dense_idf_tests.rs`: a rare-term doc ranks above a common-term doc (impossible under uniform IDF = 1.0) and the vocabulary is populated after N stores.
